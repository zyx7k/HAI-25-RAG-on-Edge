...
public:
    ...
    // Execute a batch of input samples. batchData should be a flattened
    // array of size (batchSize * per_sample_input_elements). The returned
    // scores vector will contain (batchSize * per_sample_output_elements)
    // in row-major order (batch-major): scores[i * per_output + j].
    void executeBatch(const std::vector<float>& batchData, size_t batchSize, std::vector<float>& scores);
