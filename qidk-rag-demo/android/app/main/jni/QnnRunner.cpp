...
void QnnRunner::createContext() {
    ...
    if (!file) throw std::runtime_error("Cannot open context binary: " + m_contextBinaryPath);
...
void QnnRunner::setupGraph() {
    ...
    if (m_qnnInterface->QNN_INTERFACE_VER_NAME.graphRetrieve(m_contextHandle, name, &m_graphHandle) != QNN_SUCCESS)
...
void QnnRunner::setupTensors() {
    ...
    // Typical embedding model dimensions
    uint32_t input_dim = 128;
    uint32_t output_dim = 10000;
    ...
    QNN_LOG("INFO", "Tensors configured and buffers allocated.");
}
...
void QnnRunner::execute(const std::vector<float>& query, std::vector<float>& scores) {
    if (query.size() * sizeof(float) != m_inputBufferSize)
        throw std::runtime_error("Query size mismatch: expected " +
                                 std::to_string(m_inputBufferSize / sizeof(float)) + ", got " +
