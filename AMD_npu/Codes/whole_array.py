...
def main():
    ...
    argparser.add_argument("--n-aie-rows", type=int, choices=[1, 2, 4, 8], default=1)
...
def my_matmul(
    ...
    def device_body():
        ...
        # ---- Compute hardware output memory layout ----
        TM = m           # per core tile M dimension
        TN = n           # per core tile N dimension
        
        tiles_per_row = N // TN
        tiles_per_col = M // TM
        
        dtype_bytes = np.dtype(dtype_out).itemsize
        tile_elems = TM * TN
        tile_bytes = tile_elems * dtype_bytes
        
        # Hardware aligns each logical row of tiles to 4KB boundary
        row_stride_bytes = ((tile_bytes * tiles_per_row + 4095) // 4096) * 4096
        
        # Convert byte stride to number of output elements
        row_stride_elems = row_stride_bytes // dtype_bytes
        
        # Final allocated output size the kernel will fill
        total_C_elems = row_stride_elems * tiles_per_col

        #total_C_elems = (row_stride_bytes // np.dtype(dtype_out).itemsize) * (M // m)
        @runtime_sequence(
            np.ndarray[(M * K,), np.dtype[dtype_in]],
            np.ndarray[(K * N,), np.dtype[dtype_in]],
            np.ndarray[(total_C_elems,), np.dtype[dtype_out]],
        )
        def sequence(A, B, C):
            ...
            for tb in range(ceildiv(M // m // n_aie_rows, tb_max_n_rows)):
                for pingpong in [0, 1]:
                    ...
                    for col in range(n_aie_cols):
                        ...
                        #      ----------------
                        if not c_col_maj:
                            C_row_offset = row_base * TM * row_stride_elems
                            C_col_offset = col * TN
                            ...
                            C_strides = [
                                TM * row_stride_elems,     # next output-block row
                                TN * n_aie_cols,           # next tile in row
                                row_stride_elems,          # next logical row (stride padded)
                                1
                            ]
                        else:
