...
#ifndef DTYPE_IN
#define DTYPE_IN std::int16_t
#endif
#ifndef DTYPE_OUT
#define DTYPE_OUT std::int32_t
#endif
#ifndef DTYPE_ACC
#define DTYPE_ACC int32_t
#endif
...
// =============================================================
// Allocate host vectors AVec and BVec (needed for verify/print)
// =============================================================
std::vector<A_DATATYPE> AVec(A_VOLUME);
std::vector<B_DATATYPE> BVec(B_VOLUME);

// =============================================================
// Load A.bin into AVec
// =============================================================
{
    std::ifstream fa("A.bin", std::ios::binary);
    if (!fa) {
        std::cerr << "Error: could not open A.bin\n";
        return 1;
    }
    fa.read(reinterpret_cast<char*>(AVec.data()), A_SIZE);
    if (!fa) {
...
// =============================================================
// Load B.bin into BVec
// =============================================================
{
    std::ifstream fb("B.bin", std::ios::binary);
    if (!fb) {
        std::cerr << "Error: could not open B.bin\n";
        return 1;
    }
    fb.read(reinterpret_cast<char*>(BVec.data()), B_SIZE);
    if (!fb) {
...
std::cout << "Loaded AVec and BVec from A.bin and B.bin\n";
...
std::cout << "Copied to device buffers.\n";

std::cout << "Loaded A from A.bin\n";
std::cout << "Loaded B from B.bin\n";
  ...
  for (unsigned iter = 0; iter < num_iter; iter++) {
    ...
    if (do_verify) {
    // Copy raw device output into CVec
    memcpy(CVec.data(), bufOut, (CVec.size() * sizeof(C_DATATYPE)));

    // ------------------------------------------------------------
    // Convert tile-major NPU output → flat row-major
    // ------------------------------------------------------------

    const int TM = 32;  // your tile M
    const int TN = 64;  // your tile N

    std::vector<C_DATATYPE> CRowMajor(M * N);

    int tiles_per_row = N / TN;
    int tiles_per_col = M / TM;

    for (int tile_m = 0; tile_m < tiles_per_col; tile_m++) {
        for (int tile_n = 0; tile_n < tiles_per_row; tile_n++) {

            // tile index inside CVec
            int tile_index = tile_m * tiles_per_row + tile_n;
            int tile_offset = tile_index * TM * TN;

            // unpack tile: col-major inside each tile
            for (int im = 0; im < TM; im++) {
                for (int in = 0; in < TN; in++) {

                    int src = tile_offset + in * TM + im;
    ...
    // Overwrite CVec so verification sees proper row-major output
    CVec = CRowMajor;

    // Optional: write result to disk
    std::ofstream fc("C_output.bin", std::ios::binary);
    fc.write(reinterpret_cast<char*>(CRowMajor.data()),
             CRowMajor.size() * sizeof(C_DATATYPE));

    // ------------------------------------------------------------
    // Perform verification using CVec (now row-major)
    // ------------------------------------------------------------
    if (verbosity >= 1) {
        if (do_verify_stochastic)
            std::cout << "Verifying (stochastic)..." << std::endl;
        else
            std::cout << "Verifying full matmul..." << std::endl;
    }
